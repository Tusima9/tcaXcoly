...
public class Grids : MonoBehaviour
{
   public enum PieceType
    ...
    public bool FillStep()
    {
        ...
        for (int y= yDim - 2; y >= 0; y--)
        {
            for(int loopX= 0; loopX < xDim; loopX++)
            {
                ...
                if (piece.IsMovable())
                {
                    GamePieces pieceBellow = pieces[x, y + 1];
        ...
        for(int x= 0; x < xDim; x++)
        {
            GamePieces pieceBellow = pieces[x, 0];
            if(pieceBellow.Type == PieceType.EMPTY)
            {
                Destroy(pieceBellow.gameObject);
    ...
    public void EnteredPiece(GamePieces piece)
    {
        enteredPiece = piece;
    }
    ...
    public List<GamePieces>GetMatch(GamePieces piece, int newX, int newY)
    {
        ...
        if (piece.IsColored())
        {
            ...
            List<GamePieces> matchPieces = new List<GamePieces>();
